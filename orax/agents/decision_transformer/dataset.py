...
def get_observation_mean_std(episode_dataset):
    ...
    return running_statistics.NestedMeanStd(mean=observation_mean, std=observation_std)
...
def transform_decision_transformer_input(
    ...
    *,
    ...
    def add_return_to_go(episode):
        ...
        # Scale RTG
        steps["return_to_go"] = returns / scale
    ...
    def pad_steps(steps, max_len):
        ...
        mask = _pad_along_axis(
            tf.ones(tf.shape(steps["reward"])[0], dtype=bool), max_len, 0, False
        )
        return {
