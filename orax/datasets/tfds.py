...
CHANGE: Fixes issues with inlining large dataset.
"""
...
def _dataset_size_upperbound(dataset: tf.data.Dataset) -> int:
    ...
    return tf.cast(dataset.batch(1000).reduce(0, lambda x, step: x + 1000), tf.int64)
...
def get_tfds_dataset(
    ...
    return rlds.transformations.map_steps(batched_steps, _batched_step_to_transition)
...
class JaxInMemoryRandomSampleIterator(Iterator[Any]):
    ...
    def __init__(
        ...
        self._dataset_size = jax.tree_flatten(jax.tree_map(lambda x: x.shape[0], data))[
            0
        ][0]
        ...
        if should_pmap:
            ...
            logging.info(
                "Dataset after padding %s", jax.tree_map(lambda x: x.shape, data)
            )
        ...
        self._key = (
            jnp.stack(jax.random.split(key, len(device))) if should_pmap else key
        )

        def sample_per_shard(data: Any, key: jnp.ndarray) -> Tuple[Any, jnp.ndarray]:
            key1, key2 = jax.random.split(key)
            indices = jax.random.randint(
                key1,
                (batch_size // len(device),),
                minval=0,
        ...
        if should_pmap:
            ...
            def sample_and_postprocess(
                jax_dataset, key: jnp.ndarray
            ) -> Tuple[Any, jnp.ndarray]:
                data, key = pmapped_sample(jax_dataset, key)
            ...
            self._sample = jax.jit(sample_per_shard)
