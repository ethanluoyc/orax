...
_DRIVE_PATH = 'https://drive.google.com/uc?export=download&id=1xE882IuQkXUuaeXHInYaP9eqvhQm48Et'
_DATA_PATHS = {
...
def build_episode(demo_file: os.PathLike):
  ...
  steps_dict = {
      'observation': {
          "states":
              np.concatenate([traj['obs'], [traj['nobs'][-1]]], axis=0),
          "pixels":
              np.transpose(
                  np.concatenate([traj['pixel_obs'], [traj['pixel_nobs'][-1]]],
                                 axis=0), [0, 2, 3, 1]),
      },
      'action':
          np.concatenate([traj['action'], [np.zeros_like(traj['action'][0])]],
                         axis=0),
      'reward':
          np.concatenate(
              [np.asarray(traj['reward']), [np.zeros_like(traj['reward'][0])]],
              axis=0),
      'is_first':
          np.asarray([True] + [False] * episode_length),
      'is_last':
          np.asarray([False] * episode_length + [True]),
      'is_terminal':
          np.asarray([False] * episode_length + [ends_in_terminal]),
  }
...
class Gwil(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager: tfds.download.DownloadManager):
    ...
    path = dl_manager.download_and_extract({'file_path': _DRIVE_PATH
                                           })['file_path']
