...
from orax.agents.calql.networks import CQLNetworks
from orax.agents.calql.networks import apply_and_sample_n
...
class CalQLLearner(acme.Learner):
    """CQL learner.
    ...
    def __init__(
        ...
        def sac_critic_loss(
            ...
            if max_target_backup:
                ...
                next_actions, _ = apply_and_sample_n(
                    key,
                    networks,
                    policy_params,
                ...
                next_q = batched_critic(
                    next_actions,
                    target_critic_params,
                    transitions.next_observation,
                )
                next_v = jnp.max(jnp.min(next_q, axis=-1), axis=0)
                target_q = jax.lax.stop_gradient(
                    transitions.reward * reward_scale
                    + transitions.discount * discount * next_v
                )
                chex.assert_shape(next_v, (batch_size,))
        ...
        def cql_critic_loss(
            ...
            def sampled_q(obs, key):
                ...
                # (N, B, Q)
                q = batched_critic(actions, critic_params, transitions.observation)
                # Enable Cal-QL
                if use_calql:
                    mc_returns = transitions.extras["mc_return"]
                    lower_bounds = jnp.reshape(mc_returns, (1, batch_size, 1))
                    q = jnp.maximum(q, lower_bounds)

                return q - jax.lax.stop_gradient(jnp.expand_dims(log_probs, -1))
            ...
            lse_q = jax.nn.logsumexp(combined, axis=0)
    ...
    def step(self):
        ...
        if elapsed_time > 0:
            metrics["steps_per_second"] = self._num_sgd_steps_per_step / elapsed_time
