...
Note: Different from what's used in Acme
The aggregation is done via concatenate instead reshape.
This ensures that channels from the same image is contiguouse in the flattened representation
This ensures that channels from the same image is consecutive
Fill the stack with images from the first frame if empty.
This is consistent with the frame stacking used in DrQV2
"""
...
class FrameStacker:
    ...
    def step(self, frame: np.ndarray) -> np.ndarray:
        ...
        if not self._stack:
            # Fill with the same frame if empty
        ...
        if not self._flatten:
            ...
            # Ensure that the a single frame is consecutive
