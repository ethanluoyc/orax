...
ActorId = int
InferenceServer = inference_server_lib.InferenceServer[actor_core.SelectActionFn]
...
def make_distributed_experiment(
    ...
    *,
    ...
    def build_actor(
        ...
        adder = experiment.builder.make_adder(replay, environment_spec, policy_network)
        actor = experiment.builder.make_actor(
            actor_key, policy_network, environment_spec, variable_source, adder
        )

        # Create logger and counter.
        counter = counting.Counter(counter, "actor")
        logger = experiment.logger_factory("actor", counter.get_steps_key(), actor_id)
    ...
    if experiment.max_num_actor_steps is not None:
        program.add_node(
            lp.CourierNode(
                lp_utils.StepsLimiter, counter, experiment.max_num_actor_steps
            ),
            label="counter",
        )
    ...
    if multithreading_colocate_learner_and_reverb:
        program.add_node(
            lp.MultiThreadingColocation([learner_node, replay_node]), label="learner"
        )
    else:
        ...
        with program.group("learner"):
            ...
            for _ in range(1, num_learner_nodes):
                ...
                variable_sources.append(
                    program.add_node(
                        lp.CourierNode(
                            build_learner, learner_key, replay, primary_learner=learner
                        )
                    )
                )
